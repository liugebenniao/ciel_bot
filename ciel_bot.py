...
def is_goodbye_message(message):
    return any(word in message for word in GOODBYE_KEYWORDS)
...
@bot.tree.command(name="toggle_conversation", description="一時的に会話のやりとりをオン／オフ", guild=discord.Object(id=GUILD_ID))
async def toggle_conversation(interaction: discord.Interaction):
    state = memory.get("conversation_enabled", True)
    memory["conversation_enabled"] = not state
    save_memory(MEMORY_FILE, memory)
    status = "オン" if memory["conversation_enabled"] else "オフ"
    await interaction.response.send_message(f"会話モードを{status}に切り替えました")
...
async def on_message(message):
    ...
    if not memory.get("conversation_enabled", True):
        return
...
if __name__ == "__main__":
    try:
        ...
        print(f"Bot Error: {e}")
        asyncio.run(announce(f"[ERROR] Bot起動に失敗: {e}"))
